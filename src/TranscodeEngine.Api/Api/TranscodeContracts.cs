...
/// <c>copy</c> to remux the video untouched. <see cref="MaxHeight"/> downscales to that height (omit to keep
/// the source resolution). <see cref="AudioStreamIndexes"/>/<see cref="SubtitleStreamIndexes"/> are absolute
/// input stream indices to copy (omit to copy all of that type); <see cref="DefaultAudioStreamIndex"/>/
/// <see cref="DefaultSubtitleStreamIndex"/> mark one mapped track as the default.
/// </summary>
